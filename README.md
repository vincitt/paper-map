...
Known v1.1 follow-ups: deeper in-panel field editing (author rows, provenance editor, in-app lineage-link autocomplete), full Settings/Help panels, on-focus external-change detection, and hiding the status pill in read-only browse mode.
...
- **With any LLM** — paste a paper's text plus the enrichment prompt (the **?** help panel has a **Copy prompt** button) into Claude or any model; it returns a ready-to-save record. Every AI-suggested tag carries a **source quote** (provenance) so you can verify it in seconds.
